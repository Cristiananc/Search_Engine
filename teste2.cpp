...
public:
		...
		for (int i = 0; i < word.length() ; i++){
            if(i == 0){}
                ...
                (p->documents).push_back(docId);
                 p = pRoot;
                //else if(word.size() >= 5 && word.substr(0,4) == " id=") {docId = stoi(word.substr(4,i-4)); b=true; p = pRoot;}
            }
	...
	void leitura(string texto,int tf, int t) {
        ...
        arquivo.close();
        cout << "texto "  << tf << "  " << endl;
        if (tf == 135){ cout <<((float)clock() - (float)t)/CLOCKS_PER_SEC << endl;}
    }
